...
on_win_compile_path = "C:/Program Files (x86)/Microsoft Visual Studio/2019/Community/VC/Auxiliary/Build/"
...
@invoke.task()
def build_cmult(c, path=on_win_compile_path):
    ...
    if on_win:
        if not path:    
            ...
            path = f'"{path}vcvars64.bat" x64'  # Enter the VS venv
            path += f'&& cd "{os.getcwd()}"'  # Change to current dir
            path += "&& cl /LD cmult.cpp"  # Compile
...
path_python = "C:/workplace/workplace_inMac/workplace/practicePython/python-bindings/overview_article/env_cpp2py/include/site/"
def compile_python_module(cpp_name, extension_name):
    invoke.run(
        "g++ -O3 -Wall -Werror -shared -std=c++11 -fPIC "
        "'python -m pybind11 --includes' "
        "-I C:/workplace/workplace_inMac/workplace/practicePython/python-bindings/overview_article/env_cpp2py/include/site/python3.8 -I . "
        "{0} "
        "-o {1}'python3.7-config --extension-suffix' "
        "-L. -lcppmult -Wl,-rpath,.".format(cpp_name, extension_name)
    )
    # -DMS_WIN64
    # C:/workplace/workplace_inMac/workplace/practicePython/python-bindings/overview_article/env_cpp2py/include/site/python3.8
